// Package validator_test contains tests for the content validation stability checker.
// These tests verify that the stability checker correctly:
// - Detects when content hasn't changed meaningfully (normalization)
// - Identifies validation loops where content cycles between states
// - Handles various formatting differences (whitespace, bullet styles, etc.)
package validator
...
func TestStabilityChecker(t *testing.T) {
	// Note: These tests check custom struct fields (IsStable, IsInLoop) so plain assertions are appropriate.
	// If we wanted to use testutil helpers, we could do something like:
	// testutil.AssertEqual(t, result.IsStable, tt.expectStable, "stability check")
	// However, the current approach is cleaner for this specific use case.
	tests := []struct {
		name         string
		original     string
		validated    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create a fresh checker for each test case to avoid state interference
			...
			if result.IsStable != tt.expectStable {
				...
				t.Logf("Original normalized: %q", result.NormalizedOriginal)
				t.Logf("Validated normalized: %q", result.NormalizedValidated)
			}
...
// where content cycles through multiple states before repeating.
// This simulates scenarios where validation creates a chain: A -> B -> C -> B -> C
func TestLoopDetection(t *testing.T) {
	...
	for i, content := range contents {
		...
		if i < 3 {
			...
			// Check loop length
			// At iteration 3: First modification seen at history[0], loop length = 2
			// At iteration 4: Second modification seen at history[1], loop length = 2
			// At iteration 5: First modification seen at history[0], loop length = 4
			var expectedLoopLength int
			switch i {
			case 3, 4:
				expectedLoopLength = 2
			case 5:
				expectedLoopLength = 4
			default:
				continue
			}
			if result.LoopLength != expectedLoopLength {
...
// due to LLM inconsistency in interpreting modal verbs (SHOULD/MUST/MAY).
// This test simulates the real-world scenario where validation suggestions flip-flop
// between "enforces" and "implements" based on subtle interpretation differences.
...
// mockValidateContent simulates the flip-flop behavior seen in problematic LLM validation loops.
// It deterministically alternates between suggesting "enforces" vs "implements" based on content hash,
// mimicking how LLMs can inconsistently interpret spec language like SHOULD/MUST.
...
// showContentDiff displays line-by-line differences between two content strings.
// This helps visualize exactly what changed between validation iterations,
// making it easier to debug circular validation issues.
...
// hashString creates a deterministic hash for consistent mock behavior across test runs.
// This ensures our flip-flop simulation is reproducible for debugging.
...
// TestReset verifies that the Reset method properly clears history
func TestReset(t *testing.T) {
	checker := NewContentStabilityChecker()

	// Add some history
	checker.CheckStability("content1", "content2")
	checker.CheckStability("content2", "content3")
	checker.CheckStability("content3", "content1")

	// Should detect a loop now
	result := checker.CheckStability("content1", "content2")
	if !result.IsInLoop {
		t.Error("Expected loop detection before reset")
	}
	...
	result = checker.CheckStability("content1", "content2")
	if result.IsInLoop {
		t.Error("Expected no loop detection after reset")
	}
}
